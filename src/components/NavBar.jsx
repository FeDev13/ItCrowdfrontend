import { useState } from "react";
import { Bars3BottomRightIcon, XMarkIcon } from "@heroicons/react/24/solid";
import { GiTennisCourt } from "react-icons/gi";
import { Link } from "react-router-dom";

export const NavBar = () => {
  let Links = [
    { name: "Home", link: "/" },
    { name: "Products", link: "/" },
    { name: "About", link: "/" },
  ];
  let [open, setOpen] = useState(false);
  return (
    <nav className="shadow-md w-full fixed top-0 left-0 z-10">
      <div className="md:flex items-center justify-between bg-white py-4 md:px-10 px-7">
        {/* logo section */}
        <div className="font-bold text-2xl cursor-pointer flex items-center gap-1">
          <GiTennisCourt />
          <Link to="/">
            <span className=" text-black text-3xl mx-8">E-Grip</span>
          </Link>
        </div>
        {/* Menu icon */}
        <div
          onClick={() => setOpen(!open)}
          className="absolute right-8 top-6 cursor-pointer md:hidden w-7 h-7"
        >
          {open ? <XMarkIcon /> : <Bars3BottomRightIcon />}
        </div>
        {/* linke items */}
        <ul
          className={`md:flex md:items-center md:pb-0 pb-12 absolute md:static bg-white md:z-auto z-[-1] left-0 w-full md:w-auto md:pl-0 pl-9 transition-all duration-500 ease-in ${
            open ? "top-12" : "top-[-490px]"
          }`}
        >
          {Links.map((link, id) => (
            <li key={id} className="md:ml-8 md:my-0 my-7 font-semibold">
              <a
                href={link.link}
                className="text-black hover:text-blue-400 duration-500"
              >
                {link.name}
              </a>
            </li>
          ))}
          <Link to="/login">
            <button className="btn bg-orange-700 text-white md:ml-8 font-semibold px-3 py-1 rounded duration-500 md:static">
              Login
            </button>
          </Link>
        </ul>
        {/* button */}
      </div>
    </nav>
  );
};
